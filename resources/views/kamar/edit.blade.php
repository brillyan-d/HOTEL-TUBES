...
@section('page_title', 'Edit Kamar: ' . $room->nama_kamar)

@section('content')
<div class="container mx-auto">
    <div class="mb-6">
        <h2 class="text-3xl font-bold text-gray-800">Edit Kamar: {{ $room->nama_kamar }}</h2>
        <p class="text-gray-500">Perbarui detail kamar hotel.</p>
    </div>

    <div class="bg-white p-8 shadow-lg rounded-xl">
        
        <form action="{{ route('kamar.update', $room->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
            @method('PUT')

            <div>
                <label for="nama_kamar" class="block text-sm font-medium text-gray-700 mb-1">Nama Kamar (e.g., Deluxe King, Suite)</label>
                <input type="text" id="nama_kamar" name="nama_kamar" value="{{ old('nama_kamar', $room->nama_kamar) }}" required
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <select id="tipe_kamar" name="tipe_kamar" required
                        ...
                        <option value="Single" {{ old('tipe_kamar', $room->tipe_kamar) == 'Single' ? 'selected' : '' }}>Single</option>
                        <option value="Double" {{ old('tipe_kamar', $room->tipe_kamar) == 'Double' ? 'selected' : '' }}>Double</option>
                        <option value="Suite" {{ old('tipe_kamar', $room->tipe_kamar) == 'Suite' ? 'selected' : '' }}>Suite</option>
                    </select>
                    @error('tipe_kamar')
                        <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="number" id="harga" name="harga" value="{{ old('harga', $room->harga) }}" required
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="number" id="stok" name="stok" value="{{ old('stok', $room->stok) }}" required
                           class="mt-1 block w-full border-gray-300 rounded-lg shadow-sm focus:border-red-500 focus:ring-red-500 p-3" min="0">
                    @error('stok')
                        <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    <label for="gambar" class="block text-sm font-medium text-gray-700 mb-1">Foto Kamar (Kosongkan jika tidak diubah)</label>
                    ...
                    @if($room->gambar)
                        <div class="mt-4">
                            ...
                            <img src="{{ asset('storage/' . $room->gambar) }}" alt="Gambar Kamar Saat Ini" class="w-32 h-24 object-cover rounded-lg shadow-md mt-1">
                        </div>
                    @endif
                </div>
            </div>
            ...
            <div>
                ...
                <textarea id="deskripsi" name="deskripsi" rows="4"
                          class="mt-1 block w-full border-gray-300 rounded-lg shadow-sm focus:border-red-500 focus:ring-red-500 p-3">{{ old('deskripsi', $room->deskripsi) }}</textarea>
                @error('deskripsi')
                    <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                @enderror
            </div>
